...
class Video(object):
    def __init__(self, name, root, video_dir, init_rect, img_names, gt_rect, attr, load_img=False):
        ...
        if load_img:
            ...
            if imageFilter in self.img_names[0]:
                img = cv2.imread(self.img_names[0], cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)
                img = X2Cube(img)
    ...
    def load_img(self):
        if self.imgs is None:
            ...
            if imageFilter in self.img_names[0]:
                ...
                self.imgs = [X2Cube(cv2.imread(x, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)) for x in self.img_names]
                self.width = self.imgs[0].shape[1]
                self.height = self.imgs[0].shape[0]
    ...
    def __iter__(self):
        for i in range(len(self.img_names)):
            if self.imgs is not None:
                ...
                if imageFilter in self.img_names[i]:
                    img = cv2.imread(self.img_names[i], cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)
                    img = X2Cube(img)
