...
class SiamCARTracker(SiameseTracker):
    ...
    def init(self, img, bbox, hp):
        """
        args:
            img(np.ndarray): BGR image
            bbox: (x, y, w, h) bbox
        """
        self.center_pos = np.array([bbox[0] + (bbox[2] - 1) / 2, bbox[1] + (bbox[3] - 1) / 2])  # center
        self.size = np.array([bbox[2], bbox[3]])  # w h

        # get z crop size
        w_z = self.size[0] + hp['context_amount'] * np.sum(self.size)
        h_z = self.size[1] + hp['context_amount'] * np.sum(self.size)
        s_z = round(np.sqrt(w_z * h_z))

        # get channel average value
        self.channel_average = np.mean(img, axis=(0, 1))

        # get cropped image: z_crop
        z_crop = self.get_subwindow(img, self.center_pos, cfg.TRACK.EXEMPLAR_SIZE, s_z, self.channel_average)

        # extract feature from template z_crop
    ...
    def track(self, img, hp):
        ...
        if cfg.BS.SELECT:
            if outputs['bs_order'] is not None:
                bs_order = outputs['bs_order']
                w_std = outputs['w_std']
        ...
        return {'bbox': bbox, 'bs_order': bs_order, 'w_std': w_std}
