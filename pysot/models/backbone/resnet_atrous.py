...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        x = self.conv1(x)  # [8, 3, 127, 127] --> [8, 64, 61, 61]
        x = self.bn1(x)
        x_ = self.relu(x)
        x = self.maxpool(x_)  # x_[8, 64, 61, 61] --> x[8, 64, 31, 31]

        p1 = self.layer1(x)  # [8, 64, 31, 31] --> [8, 256, 31, 31]
        p2 = self.layer2(p1)  # [8, 256, 31, 31] --> [8, 512, 15, 15]
        p3 = self.layer3(p2)  # [8, 512, 15, 15] --> [8, 1024, 15, 15]
        p4 = self.layer4(p3)  # [8, 1024, 15, 15] --> [8, 2048, 15, 15]
        out = [x_, p1, p2, p3, p4]  # 所以layer2 3 4的输出都为15 * 15 size
