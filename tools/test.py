...
parser = argparse.ArgumentParser(description='phtrack tracking')
...
parser.add_argument('--snapshot', type=str, default='snapshot/checkpoint_e16.pth', help='snapshot of models to eval')

parser.add_argument('--config', type=str, default='../experiments/phtrack/config.yaml', help='config file')
...
def main():
    ...
    tracker = phtrackTracker(model, cfg.TRACK)
        ...
        for idx, (img, gt_bbox) in enumerate(video):
            ...
            if args.vis and idx > 0:
                ...
                if not any(map(math.isnan, gt_bbox)):
                    ...
                    cv2.rectangle(img_copy, (gt_bbox[0], gt_bbox[1]),
                                  (gt_bbox[0] + gt_bbox[2], gt_bbox[1] + gt_bbox[3]), (0, 255, 0), 1)

                    cv2.rectangle(img_copy, (pred_bbox[0], pred_bbox[1]),
                                  (pred_bbox[0] + pred_bbox[2], pred_bbox[1] + pred_bbox[3]), (0, 255, 255), 1)

                    cv2.putText(img_copy, str(idx), (40, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 1)
                    cv2.imshow(video.name, img_copy)
                    cv2.waitKey(1)
