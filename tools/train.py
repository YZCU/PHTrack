...
parser = argparse.ArgumentParser(description='siamcar training')
parser.add_argument('--seed', type=int, default=123456, help='random seed')
parser.add_argument('--local_rank', type=int, default=0, help='compulsory for pytorch launcer')
# yaml文件
parser.add_argument('--cfg', type=str, default='../experiments/siamcar_r50/config.yaml', help='configuration')
...
def build_opt_lr(model, current_epoch=0):
    ...
    logger.info('Trainable layers:{}'.format(params_train))
...
def train(train_loader, model, optimizer, lr_scheduler, tb_writer):
    ...
    num_per_epoch = len(train_loader.dataset) // cfg.TRAIN.EPOCH // (cfg.TRAIN.BATCH_SIZE * world_size)  # 每epoch采样数
